...
- [ ] 1.3 Write FACTORY.md's expedite section (a §4a beside the gates):
...
- [ ] 1.4 Update FACTORY.md §3 (state table: `factory:epic-ready` row
...
- [ ] 2.1 Refactor the gate merge-and-flip routine (find gate PR, §6b
...
- [ ] 2.2 Implement the `labeled`/`unlabeled` handling for
      ...
      engines.
- [ ] 2.3 Implement expedite resolution for task events (epic via task
...
- [ ] 2.4 Implement gate GS: `Approved` comment at `factory:epic-ready`
...
- [ ] 2.5 Add conformance fixtures for every scenario in the three delta
...
- [ ] 3.1 Replace `architect-chain` with a generalized `expedite-chain` job:
...
- [ ] 3.2 Implement the missing-PAT degradation: say-once comment naming the
      token requirement and the manual control, job ends green; verify the
      say-once marker prevents repeats.
- [ ] 3.3 Guard against dispatch storms: only redispatch for issues whose
...
- [ ] 4.1 Extend `chain_node` with the auto-advance map (task hops, G1/G2
...
- [ ] 4.2 Replace the fixed `MAX_ROUNDS = 4` with a per-execution budget
...
- [ ] 5.1 Update `commands/release.md`: phase 1 flips the epic to
...
- [ ] 5.2 Update `commands/dispatch.md` (flip `factory:epic-ready` when a
...
- [ ] 6.1 Add the `factory:epic-ready` entry to `handbook/next-step.json`
