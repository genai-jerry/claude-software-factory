...
A consuming repo holds six files and none of them are logic. Everything that
decides — the router, the nine role prompts, the guards — lives in
...
flowchart TB
  ...
  EV --> STUB["caller stub<br/>15 lines · triggers and version pin only"]
  ...
  SNAP --> RUN["run role<br/>prompt + repo profile + allowed tools"]
  ...
  class STUB,ROUTE,SNAP,RUN,OUT,CHAIN core
...
flowchart LR
  ...
  K -- "issues.opened" --> RI["role = intake"]
  ...
  K -- "issue_comment" --> C2{"body is<br/>Approved?"}
  C2 -- no --> N2["no route"]
  ...
  class K,C1,C2,C3 q
  class RI,RM,RD,RP,RS,RX r
  class N1,N2,IN n
  class RE w
```

Commenting **Approved** does something in exactly three states. Anywhere else
  ...
  workflow_dispatch:
    inputs:
      ...
      role:         { description: "Factory role to run",  required: true, type: choice,
                      options: [intake, planner, architect, dispatch, implementer,
...
- **A called workflow cannot declare its own triggers.** Events are the one
  ...
  like `closed`, is a change to the stub and not to the pipeline body.
...
The nine prompts are **byte-identical in every repo**. Everything a role needs to
...
**Guard.** When **Approved** routes nowhere, the router replies naming the three
states where it works and the state the issue is actually in. Same for a reply
on a `factory:blocked` issue whose stage has no automatic resume step.
